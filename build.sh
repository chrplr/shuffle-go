...
go build -ldflags="$LDFLAGS" -o shuffle-cli cmd/shuffle-cli/main.go
...
go build -ldflags="$LDFLAGS" -o shuffle-gui cmd/shuffle-gui/main.go
...
if command -v fyne-cross >/dev/null 2>&1; then
    ...
    # fyne-cross linux -arch amd64 -name shuffle-gui -ldflags "$LDFLAGS" cmd/shuffle-gui/main.go
    # fyne-cross windows -arch amd64 -name shuffle-gui -ldflags "$LDFLAGS" cmd/shuffle-gui/main.go
